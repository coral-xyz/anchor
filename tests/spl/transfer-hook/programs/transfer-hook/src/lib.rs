...
declare_id!("HZssC2RhNw8MAhdxkCyqUNBRxP4NiMoq3hR65rvUJ3iZ");
...
pub struct Execute<'info> {
    ...
    #[account(
        seeds = [b"secondary_authority", source_account.key().as_ref()],
        bump,
    )]
    ...
    #[account(
        seeds = [source_account.key().as_ref(), amount.to_le_bytes().as_ref()],
        bump,
    )]
