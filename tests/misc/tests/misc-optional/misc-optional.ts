...
describe("misc-optional", () => {
  // Configure the client to use the local cluster.
  const provider = anchor.AnchorProvider.env();
  const wallet = provider.wallet as Wallet;
  anchor.setProvider(provider);
  const program = anchor.workspace.MiscOptional as Program<MiscOptional>;
  ...
  it("Can retrieve events when simulating a transaction", async () => {
    ...
    const expectedRaw = [
      "Program FNqz6pqLAwvMSds2FYjR4nKV3moVpPNtvkfGFrqLKrgG invoke [1]",
  ...
  it("Can use enum in idl", async () => {
    ...
    const event1 = resp1.events[0].data as IdlEvents<MiscOptional>["E7"];
    assert.deepEqual(event1.data.first, {});

    const resp2 = await program.methods
      .testInputEnum({ second: { x: new BN(1), y: new BN(2) } })
      .simulate();
    const event2 = resp2.events[0].data as IdlEvents<MiscOptional>["E7"];
    ...
    const resp3 = await program.methods
      ...
      .simulate();
    const event3 = resp3.events[0].data as IdlEvents<MiscOptional>["E7"];
    ...
    const resp4 = await program.methods
      ...
      .simulate();
    const event4 = resp4.events[0].data as IdlEvents<MiscOptional>["E7"];
