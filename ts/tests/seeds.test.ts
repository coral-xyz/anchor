...
describe("seeds-tests", () => {
  it("seeds test", async () => {
    ...
    //when
    const seedsOld = [payer.publicKey.toBuffer(), Buffer.from(utils.bytes.utf8.encode(stringSeed))];
    const seedsPartiallyNew = [payer.publicKey.toBuffer(), ...utils.seeds.from(stringSeed)];
