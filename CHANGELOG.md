...
* lang: Providing `bump = <target>` targets with `init` will now error. It is recommended to use `bump` without a target and access the seed inside function handlers via `ctx.bumps.get("<pda-account-name")` ([#1380](https://github.com/project-serum/anchor/pull/1380)).
