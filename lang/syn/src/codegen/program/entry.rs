...
pub fn generate(program: &Program) -> proc_macro2::TokenStream {
    ...
    let fallback_maybe = dispatch::gen_fallback(program).unwrap_or(quote! {
        Err(anchor_lang::anchor_attribute_error::error_without_origin!(anchor_lang::error::ErrorCode::InstructionMissing))
    });
    quote! {
        ...
        fn try_entry(program_id: &Pubkey, accounts: &[AccountInfo], data: &[u8]) -> anchor_lang::Result<()> {
            ...
            if *program_id != ID {
                return Err(anchor_lang::anchor_attribute_error::error_without_origin!(anchor_lang::error::ErrorCode::DeclaredProgramIdMismatch));
            }
            if data.len() < 8 {
