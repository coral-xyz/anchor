//! Type validating that the account is one of the given compatible Programs
// todo elliot - docs
use crate::error::{Error, ErrorCode};
use crate::{
    AccountDeserialize, Accounts, AccountsExit, CompatibleIds, Key, Result, ToAccountInfos, ToAccountMetas,
};
...
///
#[derive(Clone)]
pub struct CompatibleProgram<'info, T: CompatibleIds + Clone> {
    info: AccountInfo<'info>,
    _phantom: PhantomData<T>,
}

impl<'info, T: CompatibleIds + Clone + fmt::Debug> fmt::Debug for CompatibleProgram<'info, T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("CompatibleProgram").field("info", &self.info).finish()
    }
}

impl<'a, T: CompatibleIds + Clone> CompatibleProgram<'a, T> {
    fn new(info: AccountInfo<'a>) -> CompatibleProgram<'a, T> {
        Self {
            info,
    ...
    /// Deserializes the given `info` into a `CompatibleProgram`.
    #[inline(never)]
    pub fn try_from(info: &AccountInfo<'a>) -> Result<CompatibleProgram<'a, T>> {
        if !&T::is_compatible_id(info.key) {
            // todo elliot
            return Err(Error::from(ErrorCode::InvalidProgramId));
                // .with_pubkeys((*info.key, T::compatible_ids())));
        }
        if !info.executable {
            return Err(ErrorCode::InvalidProgramExecutable.into());
        }

        Ok(CompatibleProgram::new(info.clone()))
    }
...
impl<'info, T> Accounts<'info> for CompatibleProgram<'info, T>
where
    T: CompatibleIds + Clone,
{
    #[inline(never)]
    fn try_accounts(
        _program_id: &Pubkey,
        ...
        *accounts = &accounts[1..];
        CompatibleProgram::try_from(account)
    }
}

impl<'info, T: CompatibleIds + Clone> ToAccountMetas for CompatibleProgram<'info, T> {
...
impl<'info, T: CompatibleIds + Clone> ToAccountInfos<'info> for CompatibleProgram<'info, T> {
    fn to_account_infos(&self) -> Vec<AccountInfo<'info>> {
        vec![self.info.clone()]
    }
}

impl<'info, T: CompatibleIds + Clone> AsRef<AccountInfo<'info>> for CompatibleProgram<'info, T> {
    fn as_ref(&self) -> &AccountInfo<'info> {
        &self.info
    }
}

impl<'info, T: CompatibleIds + Clone> Deref for CompatibleProgram<'info, T> {
...
impl<'info, T: AccountDeserialize + CompatibleIds + Clone> AccountsExit<'info> for CompatibleProgram<'info, T> {}

impl<'info, T: AccountDeserialize + CompatibleIds + Clone> Key for CompatibleProgram<'info, T> {
    fn key(&self) -> Pubkey {
        *self.info.key
    }
}
