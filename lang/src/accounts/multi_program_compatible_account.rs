...
use crate::{AccountDeserialize, AccountSerialize, Accounts, AccountsClose, AccountsExit, Key, Result, ToAccountInfo, ToAccountInfos, ToAccountMetas, CompatibleOwners};
...
/// # Basic Functionality
///
/// MultiProgramCompatibleAccount checks that `T::is_compatible_owner(Account.info.owner)`.
/// This means that the data type that MultiProgramCompatibleAccount wraps around (`=T`) needs to
/// implement the [CompatibleOwners trait](crate::CompatibleOwners).
/// The `#[account]` attribute implements the Owner trait for
/// a struct using the `crate::ID` declared by [`declareId`](crate::declare_id)
/// in the same program. It follows that MultiProgramCompatibleAccount can also be used
/// with a `T` that comes from a different program.
///
/// Checks:
///
/// - `T::is_compatible_owner(MultiProgramCompatibleAccount.info.owner)`
...
#[derive(Clone)]
pub struct MultiProgramCompatibleAccount<'info, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone> {
    account: T,
    info: AccountInfo<'info>,
}

impl<'info, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone + fmt::Debug> fmt::Debug
for MultiProgramCompatibleAccount<'info, T>
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("MultiProgramCompatibleAccount")
...
impl<'a, T: AccountSerialize + AccountDeserialize + crate::CompatibleOwners + Clone> MultiProgramCompatibleAccount<'a, T> {
    fn new(info: AccountInfo<'a>, account: T) -> MultiProgramCompatibleAccount<'a, T> {
        Self { info, account }
    }

    /// Deserializes the given `info` into a `MultiProgramCompatibleAccount`.
    #[inline(never)]
    pub fn try_from(info: &AccountInfo<'a>) -> Result<MultiProgramCompatibleAccount<'a, T>> {
    ...
    /// Deserializes the given `info` into a `MultiProgramCompatibleAccount` without checking
    /// the account discriminator. Be careful when using this and avoid it if
    /// possible.
    #[inline(never)]
    pub fn try_from_unchecked(info: &AccountInfo<'a>) -> Result<MultiProgramCompatibleAccount<'a, T>> {
        if info.owner == &system_program::ID && info.lamports() == 0 {
            return Err(ErrorCode::AccountNotInitialized.into());
        }
        if !T::is_compatible_owner(info.owner) {
...
impl<'info, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone> Accounts<'info>
for MultiProgramCompatibleAccount<'info, T>
    where
        T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone,
{
    #[inline(never)]
    fn try_accounts(
        _program_id: &Pubkey,
        ...
        *accounts = &accounts[1..];
        MultiProgramCompatibleAccount::try_from(account)
    }
}

impl<'info, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone> AccountsExit<'info>
for MultiProgramCompatibleAccount<'info, T>
{
...
impl<'info, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone> AccountsClose<'info>
for MultiProgramCompatibleAccount<'info, T>
{
...
impl<'info, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone> ToAccountMetas
for MultiProgramCompatibleAccount<'info, T>
{
...
impl<'info, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone> ToAccountInfos<'info>
for MultiProgramCompatibleAccount<'info, T>
{
...
impl<'info, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone> AsRef<AccountInfo<'info>>
for MultiProgramCompatibleAccount<'info, T>
{
...
impl<'info, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone> AsRef<T>
for MultiProgramCompatibleAccount<'info, T>
{
    fn as_ref(&self) -> &T {
        &self.account
...
impl<'a, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone> Deref for MultiProgramCompatibleAccount<'a, T> {
    type Target = T;
...
impl<'a, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone> DerefMut for MultiProgramCompatibleAccount<'a, T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        #[cfg(feature = "anchor-debug")]
        if !self.info.is_writable {
            solana_program::msg!("The given MultiProgramCompatibleAccount is not mutable");
            panic!();
        }
        &mut self.account
    }
...
impl<'info, T: AccountSerialize + AccountDeserialize + CompatibleOwners + Clone> Key for MultiProgramCompatibleAccount<'info, T> {
    fn key(&self) -> Pubkey {
        *self.info.key
    }
}
