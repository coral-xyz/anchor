...
use crate::{
    AccountDeserialize, AccountSerialize, Accounts, AccountsExit, AnchorResult, ToAccountInfos,
    ToAccountMetas,
};
...
impl<'info, T: AccountSerialize + AccountDeserialize + Clone> CpiState<'info, T> {
    ...
    #[inline(never)]
    pub fn try_from(info: &AccountInfo<'info>) -> AnchorResult<CpiState<'info, T>> {
