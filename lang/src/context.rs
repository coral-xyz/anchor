...
use crate::{Accounts, ToAccountInfos, ToAccountMetas, TryAccountInfos};
...
where
    T: ToAccountMetas + ToAccountInfos<'info> + TryAccountInfos<'info>,
{
    pub accounts: T,
...
impl<'info, T: ToAccountInfos<'info> + ToAccountMetas + TryAccountInfos<'info>> ToAccountInfos<'info>
    for CpiContext<'_, '_, '_, 'info, T>
{
    fn to_account_infos(&self) -> Vec<AccountInfo<'info>> {
        // always uses try_account_infos in case there are optional accounts
        let mut infos = self.accounts.try_account_infos(&self.program);
...
impl<'info, T: ToAccountInfos<'info> + ToAccountMetas + TryAccountInfos<'info>> TryAccountInfos<'info>
    for CpiContext<'_, '_, '_, 'info, T>
{
...
impl<'info, T: ToAccountInfos<'info> + ToAccountMetas + TryAccountInfos<'info>> ToAccountMetas
    for CpiContext<'_, '_, '_, 'info, T>
{
...
{
    fn to_account_infos(&self) -> Vec<AccountInfo<'info>> {
        let mut infos = self.cpi_ctx.accounts.try_account_infos(&self.cpi_ctx.program);
...
#[allow(deprecated)]
impl<'a, 'b, 'c, 'info, T: Accounts<'info>> TryAccountInfos<'info>
    for CpiStateContext<'a, 'b, 'c, 'info, T>
{
    fn try_account_infos(&self, _program: &AccountInfo<'info>) -> Vec<AccountInfo<'info>> {
        let mut infos = self.cpi_ctx.accounts.try_account_infos(&self.cpi_ctx.program);
