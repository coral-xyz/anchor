...
impl anchor_lang::AccountDeserialize for TokenAccount {
    fn try_deserialize_unchecked(buf: &mut &[u8]) -> anchor_lang::Result<Self> {
        println!("invoke TokenAccount2022Compat::try_deserialize_unchecked()");
...
impl anchor_lang::AccountDeserialize for Mint {
    ...
    fn try_deserialize_unchecked(buf: &mut &[u8]) -> anchor_lang::Result<Self> {
        println!("invoke Mint2022Compat::try_deserialize_unchecked()");
        let res = spl_token_2022::extension::StateWithExtensions::<spl_token_2022::state::Mint>::unpack(buf)
            .map(|m| m.base)
            .map(Mint)
            .map_err(Into::into);
        println!("res: {:?}", res);
