...
use optional::state::{Data1, Data2};
...
// Runs a client for tests/composite.
//
...
fn optional(client: &Client, pid: Pubkey, signer: Keypair) -> Result<()> {
    ...
    // `Initialize` parameters.
    let optional_2 = Keypair::new();

    let optional2_key = optional_2.pubkey();

    let optional1_seeds = &[Data1::PREFIX.as_ref(), optional2_key.as_ref()];
    let optional_1 = Pubkey::find_program_address(optional1_seeds, &pid).0;
    ...
    program
        ...
        .instruction(system_instruction::create_account(
            ...
            program
                .rpc()
                .get_minimum_balance_for_rent_exemption(Data2::LEN)?,
            Data2::LEN as u64,
            &program.id(),
        ))
        .instruction(system_instruction::create_account(
        ...
        .accounts(OptionalInitialize {
            ...
            optional_1: Some(optional_1),
            optional_2: Some(optional_2.pubkey()),
    ...
    // Assert the transaction worked.
    let optional1_account: Data1 = program.account(optional_1)?;
    assert_eq!(optional1_account.data, value);

    let optional2_account: Data2 = program.account(optional_2.pubkey())?;
    assert_eq!(optional2_account.optional_1, optional_1);

    let required: Data2 = program.account(required.pubkey())?;
    assert_eq!(required.optional_1, Pubkey::default());
